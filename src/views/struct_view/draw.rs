...
pub fn draw_struct_view(view: &mut StructuredView) {
    ...
    let pal = palette();
    let focused = view.group.is_focused();
    ...
    for row in 0..h as usize {
        ...
        if idx >= view.visible_nodes.len() {
            view.group.buffer_mut().hline(0, y, w, ' ', normal);
            continue;
        }
...
fn draw_struct_row(
    ...
    view.group.buffer_mut().print(sep1_x, y, "│", sep_style);
    ...
    view.group.buffer_mut().print(sep2_x, y, "│", sep_style);
...
/// Position the InputLine child at the correct column and blit it.
fn blit_editor(view: &mut StructuredView, _w: u16, h: u16, cols: &ColLayout) {
    ...
    if row < view.scroll || row >= view.scroll + h as usize {
        return;
    }
    let y = (row - view.scroll) as u16;
    let sep1_x = cols.key_w as u16;
    let val_x = sep1_x + 1;
    let sep2_x = val_x + cols.val_w as u16;
    let meta_x = sep2_x + 1;
