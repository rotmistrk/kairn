...
impl EditorView {
    ...
    fn cancel_command_input(&mut self) {
        ...
        if is_search {
            ...
            if let Some((l, c)) = self.editor.incsearch_origin() {
                self.editor.set_incsearch_origin(None);
    ...
    fn submit_command_input(&mut self) {
        ...
        if self.editor.mode() == EditorMode::Search {
            if !buf.is_empty() {
                self.editor.command_history_mut().push(buf.clone());
            }
            self.editor.set_mode(EditorMode::Normal);
    ...
    fn backspace_command_input(&mut self) {
        if self.editor.command_buf().is_empty() {
            ...
            self.editor.command_buf_mut().pop();
            self.editor.set_history_index(None);
            self.update_incsearch();
        }
    }
    ...
    fn update_incsearch(&mut self) {
        ...
        let origin_off = self
            ...
            .unwrap_or(0);
        self.editor
            .set_highlight(HighlightState::build(pattern, &content, origin_off));
        // Jump cursor to current match
        if let Some(hl) = self.editor.highlight() {
            if let Some(&(start, _)) = hl.current_match() {
