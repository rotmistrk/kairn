...
pub fn run_mcp_bridge() -> io::Result<()> {
    ...
    run_io_threads(socket)
}

fn resolve_socket_path() -> io::Result<String> {
    let socket_path =
        env::var("KAIRN_MCP_SOCKET").map_err(|_| Error::new(ErrorKind::NotFound, "KAIRN_MCP_SOCKET not set"))?;
...
fn run_io_threads(socket: UnixStream) -> io::Result<()> {
