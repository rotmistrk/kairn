...
fn disambiguate_path(path: &str, others: &[&str]) -> String {
    ...
    for depth in 2..=parts.len() {
        let suffix: String = parts[..depth].iter().rev().copied().collect::<Vec<_>>().join("/");
        let unique = others.iter().all(|other| {
            let oparts: Vec<&str> = other.rsplit('/').collect();
            let osuffix: String = oparts[..depth.min(oparts.len())]
                .iter()
                .rev()
                .copied()
                .collect::<Vec<_>>()
...
impl SlottedDesktop {
    ...
    pub fn display_name(&self, slot: SlotId, idx: usize) -> String {
        ...
        let dups: Vec<&str> = s
            ...
            .iter()
            .enumerate()
            .filter(|(i, (t, _))| *i != idx && t.rsplit('/').next().unwrap_or(t) == basename)
            .map(|(_, (t, _))| t.as_str())
            .collect();
        if dups.is_empty() {
    ...
    fn draw_slot_tabs(&self, surface: &mut Surface, sid: SlotId, start_x: u16, y: u16, max_x: u16) {
        ...
        if x + display_width(&title_str, 1) > max_x {
            return;
        }
    ...
    fn draw_bottom_chrome(&self, surface: &mut Surface, rects: &[Rect; 4], bounds: Rect, tall: bool) {
