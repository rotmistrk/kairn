...
impl SlottedDesktop {
    /// Update LRU timestamp for the active tab in a slot.
    fn touch_active_tab(&mut self, slot_id: SlotId) {
        self.lru_counter += 1;
        let s = &mut self.slots[slot_id as usize];
        if s.active < s.lru.len() {
            s.lru[s.active] = self.lru_counter;
        }
    }

    /// After switching tabs, set bounds on the newly active view and select it.
    fn sync_active_bounds(&mut self, slot_id: SlotId) {
        self.touch_active_tab(slot_id);
