...
const CMD_TABLE: &[CmdEntry] = &[
    CmdEntry { full: "delete", min_abbrev: 1, id: ExCmdId::Delete },
    CmdEntry { full: "diff", min_abbrev: 3, id: ExCmdId::Diff },
    CmdEntry { full: "edit", min_abbrev: 1, id: ExCmdId::Edit },
    CmdEntry { full: "nodiff", min_abbrev: 3, id: ExCmdId::NoDiff },
    CmdEntry { full: "quit", min_abbrev: 1, id: ExCmdId::Quit },
    CmdEntry { full: "set", min_abbrev: 2, id: ExCmdId::Set },
    CmdEntry { full: "setglobal", min_abbrev: 4, id: ExCmdId::SetGlobal },
    CmdEntry { full: "substitute", min_abbrev: 1, id: ExCmdId::Substitute },
    CmdEntry { full: "write", min_abbrev: 1, id: ExCmdId::Write },
    CmdEntry { full: "wq", min_abbrev: 2, id: ExCmdId::WriteQuit },
    CmdEntry { full: "x", min_abbrev: 1, id: ExCmdId::Exit },
    CmdEntry { full: "yank", min_abbrev: 1, id: ExCmdId::Yank },
];
...
pub fn lookup_command(input: &str) -> Option<ExCmdId> {
    ...
    for entry in CMD_TABLE {
        if input.len() >= entry.min_abbrev
            && input.len() <= entry.full.len()
            && entry.full.starts_with(input)
        {
            if found.is_some() {
