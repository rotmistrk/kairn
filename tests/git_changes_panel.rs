...
fn git_changes_view_is_non_closeable() {
    ...
    h.run_cycles(1);
    // The left slot chrome shows "Files" with a count badge of 2 (Files + Git)
    let top_row = h.row(0);
    assert!(
        top_row.contains("Files") && top_row.contains("2"),
        "Left slot should show Files tab with count 2, got: {top_row}"
    );
}
